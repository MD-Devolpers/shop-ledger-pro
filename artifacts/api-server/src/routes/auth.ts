...
router.get("/auth/me", async (req, res): Promise<void> => {
  ...
  const [perms] = await db.select().from(staffPermissionsTable)
    .where(eq(staffPermissionsTable.userId, userId));

  const isAdmin = user.role === "admin";
  const permissions = {
    isAdmin,
    canSeePurchasePrice: isAdmin || (perms?.canSeePurchasePrice ?? false),
    canSeeProfit: isAdmin || (perms?.canSeeProfit ?? false),
    canSeePurchaseBills: isAdmin || (perms?.canSeePurchaseBills ?? false),
    canEditDeleteSale: isAdmin || (perms?.canEditDeleteSale ?? false),
  };
